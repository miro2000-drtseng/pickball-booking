...
// 管理員手動新增會員（不需該球友先自行註冊或用 LINE 登入）
router.post('/members/create', adminMiddleware, async (req, res) => {
  const { name, phone, email } = req.body;
  if (!name) return res.status(400).json({ error: '請填寫暱稱' });
  if (!phone && !email) return res.status(400).json({ error: '手機號碼與 Email 至少需填一項' });

  if (phone) {
    const { data: existingPhone } = await supabase.from('members').select('id').eq('phone', phone).single();
    if (existingPhone) return res.status(400).json({ error: '此手機號碼已被使用' });
  }
  if (email) {
    const { data: existingEmail } = await supabase.from('members').select('id').eq('email', email).single();
    if (existingEmail) return res.status(400).json({ error: '此 Email 已被使用' });
  }
...
router.post('/blocked-slots', adminMiddleware, async (req, res) => {
  const { label, court_ids, hour_start, hour_end, block_date_start, block_date_end, day_of_week, valid_from, valid_until } = req.body;
  ...
  if (!block_date_start && day_of_week == null) return res.status(400).json({ error: '請選擇一次性日期或每週幾' });

  // 一次性封場：起始～結束日期之間每一天各建立一筆
  if (block_date_start) {
    const start = new Date(block_date_start + 'T00:00:00Z');
    const end = block_date_end ? new Date(block_date_end + 'T00:00:00Z') : start;
    ...
    if (dates.length > 90) return res.status(400).json({ error: '一次最多封場 90 天，請縮小範圍' });

    const rows = dates.map(block_date => ({
      label: label || '封場', court_ids, hour_start, hour_end,
      block_date, day_of_week: null, valid_from: null, valid_until: null
    }));
