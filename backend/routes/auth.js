...
router.post('/admin/forgot-password', async (req, res) => {
  ...
  const { data: admin } = await supabase
    .from('admins').select('id, email').eq('email', email).single();

  const genericMsg = { ok: true, message: '若該 Email 為管理員帳號，重設密碼信件將會寄出，請查收信箱' };
  ...
  await supabase.from('password_resets').insert({
    admin_id: admin.id, token, expires_at: expiresAt
  });
...
router.post('/reset-password', async (req, res) => {
  ...
  const hash = await bcrypt.hash(password, 12);
  if (resetReq.admin_id) {
    await supabase.from('admins').update({ password_hash: hash }).eq('id', resetReq.admin_id);
  } else {
